...
func NewLearnListener() *LearnListener {
	config := getConfig()
	// rateLimiterEnabledFromConfig, err := strconv.ParseBool(config["rateLimiterEnabled"].(string))
	rateLimiterEnabledFromConfig := config["rateLimiterEnabled"].(bool)
	// if err != nil {
	// 	log.Fatal("invalid rateLimiterEnabled in config")
	// }
	// rateLimiterIntervalFromConfig, err := strconv.ParseInt(config["rateLimiterInterval"].(string), 10, 64)
	rateLimiterIntervalFromConfig := config["rateLimiterInterval"].(int)
	// if err != nil {
	// 	log.Fatal("invalid rateLimiterInterval in config")
	// }
	server := &learnServer{
		rateLimitedEventCh:    make(chan notificationWrapper, 500),
		eventID:               -1,
		sideEffectID:          -1,
		nonK8sSideEffectID:    -1,
		eventChMap:            sync.Map{},
...
func (l *LearnListener) NotifyLearnBeforeNKWrite(request *sieve.NotifyLearnBeforeNKWriteRequest, response *sieve.Response) error {
	return l.Server.NotifyLearnBeforeNKWrite(request, response)
}

func (l *LearnListener) NotifyLearnAfterNKWrite(request *sieve.NotifyLearnAfterNKWriteRequest, response *sieve.Response) error {
	return l.Server.NotifyLearnAfterNKWrite(request, response)
}
...
func (s *learnServer) NotifyLearnBeforeControllerRecv(request *sieve.NotifyLearnBeforeControllerRecvRequest, response *sieve.Response) error {
	...
	s.notificationCh <- notificationWrapper{ntype: beforeRecv, payload: fmt.Sprintf("%d\t%s\t%s\t%s", eID, request.OperationType, request.ResourceType, request.Object)}
	<-waitingCh
...
func (s *learnServer) NotifyLearnAfterControllerRecv(request *sieve.NotifyLearnAfterControllerRecvRequest, response *sieve.Response) error {
	s.notificationCh <- notificationWrapper{ntype: afterRecv, payload: fmt.Sprintf("%d", request.EventID)}
...
func (s *learnServer) NotifyLearnBeforeControllerWrite(request *sieve.NotifyLearnBeforeControllerWriteRequest, response *sieve.Response) error {
	...
	s.notificationCh <- notificationWrapper{ntype: beforeWrite, payload: fmt.Sprint(sID)}
	<-waitingCh
...
func (s *learnServer) NotifyLearnAfterControllerWrite(request *sieve.NotifyLearnAfterControllerWriteRequest, response *sieve.Response) error {
	s.notificationCh <- notificationWrapper{ntype: afterWrite, payload: fmt.Sprintf("%d\t%s\t%s\t%s\t%s\t%s", request.SideEffectID, request.SideEffectType, request.ResourceType, request.ReconcilerType, request.Error, request.Object)}
	*response = sieve.Response{Message: request.SideEffectType, Ok: true}
	return nil
}

func (s *learnServer) NotifyLearnBeforeNKWrite(request *sieve.NotifyLearnBeforeNKWriteRequest, response *sieve.Response) error {
	sID := atomic.AddInt32(&s.nonK8sSideEffectID, 1)
	waitingCh := make(chan int32)
	s.nonK8sSideEffectChMap.Store(fmt.Sprint(sID), waitingCh)
	s.notificationCh <- notificationWrapper{ntype: beforeNonK8sWrite, payload: fmt.Sprint(sID)}
	<-waitingCh
	*response = sieve.Response{Message: request.FunName, Ok: true, Number: int(sID)}
	return nil
}

func (s *learnServer) NotifyLearnAfterNKWrite(request *sieve.NotifyLearnAfterNKWriteRequest, response *sieve.Response) error {
	s.notificationCh <- notificationWrapper{ntype: afterNonK8sWrite, payload: fmt.Sprintf("%d\t%s\t%s\t%s", request.SideEffectID, request.RecvTypeName, request.FunName, request.ReconcilerType)}
...
func (s *learnServer) coordinatingEvents() {
	for {
		...
		case nw := <-s.notificationCh:
			...
			case beforeNonK8sWrite:
				if obj, ok := s.nonK8sSideEffectChMap.Load(nw.payload); ok {
					log.Printf("[SIEVE-BEFORE-NON-K8S-WRITE]\t%s\n", nw.payload)
					ch := obj.(chan int32)
					ch <- 0
				} else {
					log.Fatal("invalid object in eventChMap")
				}
			case afterNonK8sWrite:
