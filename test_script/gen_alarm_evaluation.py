...
def gen_alarm_evaluation(path: os.PathLike) -> pandas.DataFrame:
    ...
    for operator in operators:
        ...
        for test in tests:
            ...
            total_bugs = len(
                testDF[
                    (testDF["result"] == "newbug") | (testDF["result"] == "by-product")
                ]
            )
            ...
            modes = ["intermediate-state", "stale-state", "unobserved-state"]
            ...
            for mode in modes:
                ...
                mode_newbug_count = len(
                    modeDF[
                        (modeDF["result"] == "newbug")
                        | (modeDF["result"] == "by-product")
                    ]
                )
                ...
                alarm_data[
                    "# injection not started (-1) (%s)" % mode
                ] = mode_not_started_count
                alarm_data[
                    "# injection not finished (-2) (%s)" % mode
                ] = mode_not_finished_count
                alarm_data[
                    "# cmd not returned (-3) (%s)" % mode
                ] = mode_not_returned_count
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate evaluation alarm sheet from inspection results."
    )
    ...
    writer = pandas.ExcelWriter(
        "evaluation-alarm-%s.xlsx" % (test_id), engine="xlsxwriter"
    )
