...
def generate_anaylze_table(json_path):
    ...
    config_table = []
    to_fill = "fill_me"
    test_type_list = ["intermediate-state", "stale-state", "unobserved-state"]
    ...
    for operator in data:
        ...
        for workload in data[operator]:
            ...
            for test_type in test_type_list:
                ...
                time_data[make_test_key(test_type, "avg_time")] = (
                    0 if len(durations) == 0 else np.mean(durations)
                )
                alarm_data[make_test_key(test_type, "# total run")] = len(test_set)
                alarm_data[make_test_key(test_type, "# no failure (0)")] = ret_map[0]
                alarm_data[
                    make_test_key(test_type, "# injection not started (-1)")
                ] = ret_map[-1]
                alarm_data[
                    make_test_key(test_type, "# injection not finished (-2)")
                ] = ret_map[-2]
                alarm_data[
                    make_test_key(test_type, "# cmd not returned (-3)")
                ] = ret_map[-3]
                alarm_data[make_test_key(test_type, "# exception (-4)")] = ret_map[-4]
                alarm_data[
                    make_test_key(test_type, "# total alarm (alarm > 0)")
                ] = ret_map[1]
                alarm_data[make_test_key(test_type, "# flaky")] = to_fill
                alarm_data[
                    make_test_key(test_type, "# true alarm (alarm > 0)")
                ] = to_fill
                alarm_data[
                    make_test_key(test_type, "# false alarm (alarm > 0)")
                ] = to_fill
