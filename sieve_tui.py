...
def run():
        ...
        if action == "learn":
                ...
                os.system("python3 build.py -p %s -m %s -d laphets"%(operator, mode))
...
class SieveTUI:
        ...
        def __init__(self, master: py_cui.PyCUI):
                ...
                self.operator_scroll_cell = self.master.add_scroll_menu('Operator', 0, 0, row_span=6, column_span=2)
                self.workload_scroll_cell = self.master.add_scroll_menu('Workload', 0, 2, row_span=6, column_span=2)
                self.action_scroll_cell = self.master.add_scroll_menu('Action', 0, 4, row_span=6, column_span=2)

                self.operator_scroll_cell.add_item_list(controllers.test_suites.keys())

                self.operator_scroll_cell.add_key_command(py_cui.keys.KEY_ENTER, self.goto_workload)
                self.workload_scroll_cell.add_key_command(py_cui.keys.KEY_ENTER, self.goto_action)
        ...
        def goto_workload(self):
                ...
                self.workload_scroll_cell.add_item_list(controllers.test_suites[operator].keys())
